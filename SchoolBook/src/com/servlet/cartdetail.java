...
public class cartdetail extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       // processRequest(request, response);
        ...
        if(path.equals("/displaycartdetail"))
        {
        HttpSession session=request.getSession();
	LoginService u=(LoginService)session.getAttribute("LoginService");
        String uid=u.getUserID();
        CartService cs=new CartService();
        List l=(List)cs.display_cart(uid);
        request.setAttribute("l",l);
        request.getRequestDispatcher("Display_cart.jsp").forward(request, response);
        }
         if(path.equals("/createbooktable"))
        {
        HttpSession session=request.getSession();
	LoginService u=(LoginService)session.getAttribute("LoginService");
        String uid=u.getUserID();
        CartService cs=new CartService();
        String cid[]=request.getParameterValues("checkbuy");
        cs.create_booktable(uid, cid);
        response.sendRedirect("index.jsp");
        }
        if(path.equals("/deletecart"))
        {
            String cid=request.getParameter("cid");
            CartService cs=new CartService();
            cs.delete_cart(cid);
            response.sendRedirect("displaycartdetail");
        }
    }
