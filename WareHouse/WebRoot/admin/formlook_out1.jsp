<%-- 
    Document   : formlook_out1
    Created on : 2013-12-7, 14:49:08
...
<!DOCTYPE html>
<jsp:useBean id="formlook_out1" scope="session" class="com.db.mydb" />
<html>
    <head>
   ...
   <body bgcolor="#ffffff">
      ...
      <tr>
         <td width=20% align=center>出库单号</td>
    ...
    <%
           url="yes";
           formlook_out1.getConn("warehouse_m", "root","123456");
	   sql="select * from ware_out";
	   ResultSet  rs=formlook_out1.executeQuery1(sql);
	    if(rs.next()) do{ 
    %>
      <tr>
	    ...
	    <td width=15% align=center><%=rs.getString(6)%></td>
            <td width=8% align=center>
                <%if(rs.getString(7).equals(url)){  %>
                         <%="已批准"%> 
                <% } else{  %> 
                <a href="javascript:if(confirm('are you sure？')){window.open('formlook_out1_query.jsp?out_id=<%=rs.getString(1)%>','_self');}else{}">批准</a>
                <% }%>
            </td>
      </tr>
      <%
	   }while(rs.next());else{%> 
           <td colspan="7" align="center" height="50">暂时没有出库单。</td>
      <%}%>
         <% if(rs!=null) rs.close();
	    formlook_out1.closeStmt();
	    formlook_out1.closeConn();
    %>
